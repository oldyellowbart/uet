...
PE0123_as_Output_Init();//ok
PC4567_as_Input_Init();
Uart5_Init();
PLL_Init();
...
void write_UART_number(int number)
	...
	void write_uart_Str(unsigned char *str)
	{
		unsigned int e, len;
		len = strlen(str);
		for (e = 0; e < len; e++)
		{
		UART5_Transmitter(str[i]);
		}
		UART5_Transmitter('\n');
	}
...
void usdelay(int time){for (iklm = 0; iklm < time*4; iklm++);}