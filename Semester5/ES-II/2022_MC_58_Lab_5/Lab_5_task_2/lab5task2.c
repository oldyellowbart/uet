/* Lab_05_task_2 task recreate using library & Example
-----------------------------------------------------------------------
...
Task 5.2: Re-do the Example_7_2 but instead of using PortF, use any other
GPIO Port
-----------------------------------------------------------------------
...
void PLL_Init(void);
void PD1_as_M1PWM1_Init(void);
void PWM_Module1_Channel1_Init(void);
void clocks_delay(volatile uint32_t clocks);

int main(){
	PLL_Init();
	PD1_as_M1PWM1_Init();
	PWM_Module1_Channel1_Init();
	
	while(1){
		PWM1->_1_CMPA = 36335;//for averahe bldc pwm
	}
}
...
void PWM_Module1_Channel1_Init(void){
	...
	PWM1->_1_CMPA = 34605;// 34605-2ms 36572-1ms
	...
	// Step 9: Enalbe PWM Module 1 Channel 1 Output
	PWM1->ENABLE |= 1<<1;
}
