...
void timer0_Periodic(){
	...
	TIMER0->CFG |= 0<<0;					// timer0 is used as a 32-bit (concatenated) timer, i.e., timer0A
	...
	// Step 5: Interrupt configurations
		TIMER0->ICR = 0x01;					// No interrupt enabled (RO Register)
	TIMER0->IMR = 1<<0;					// Clear timer status flag (TATORIS, TATOMIS)
