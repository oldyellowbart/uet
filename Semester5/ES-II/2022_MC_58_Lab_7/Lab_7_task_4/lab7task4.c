...
-----------------------------------------------------------------------
Task 7.4: Interface a servo motor (by connecting its signal control pin)
with any PWM pin of the TIVA C Series LaunchPad and program it to control
its rotation by varying a potentiometer, interfaced to any analog pin. 
(Reference for servo control PWM signal is also attached with this assignment). 
You may need external power supply to power up the motor. 
-----------------------------------------------------------------------
Written by Haseeb-ul-hassan (github-oldyellowbart) on 09-NOV-2024.
...
int main(){
	...
	while(1){
		...
		pwm = (adc_data+36000);//0.5 to get 2048
		PWM1->_3_CMPA = pwm;//for averahe bldc pwm
	}
}
...
void PWM_Module1_Channel6_Init(void){
	...
	PWM1->_3_CMPA = 32000;// 34605-2ms 36572-1ms
