...
int main()
	...
	while(1)
		{	
		//th_rpm = (adc_data*930)/341;
			th_rpm = ((adc_data/40)*12*93)/10;
		adc_data = ( ADC0->SSFIFO3 & 0xFFF);//pwm
//		pwm = ((19000-(adc_data * 0.25)));
//		err =((th_rpm-edge_count)*100)/th_rpm;
//		adc_data2 = ( ADC1->SSFIFO0 & 0xFFF);//ADC_SS0_FIFO_DATA_R & 0xFFF
//		current = ((adc_data2-3115)*13)/1000;
//		PWM1->_3_CMPA = pwm;	
		display(err,th_rpm,edge_count,adc_data*0.5);	
//		write_UART_number(th_rpm);//ch1
//		UART5_Transmitter(',');
//		write_UART_number(edge_count);//ch2
//		UART5_Transmitter('\r\n');//next line
		msdelay(30);
...
void write_LCD_number(float number)
{
    ...
    // First, display the integer part (same logic as before)
    int length = calculateLength(integer_part);
