...
int main()
	...
	while(1)
		...
		PWM1->_3_CMPA = pwm;	
		display(err,th_rpm,edge_count,current);	
			write_UART_number(edge_count);
			UART5_Transmitter('\n');
			msdelay(30);
	}
	}
...
void write_LCD_Char(int data)
{
	...
	msdelay(125);										// Step 6. milisecond delay to match lcd speed of writing data	(__delay__)
